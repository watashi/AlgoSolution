...
fn main() {
    ...
    for _ in 0..re {
        ...
        for x in 1..=n {
            let mut y = info[0].1;
            for i in x + 1..chain.len() {
                let m = (x + i + 1) / 2;
                let mut yy = (chain.len() - i - 1 + x) as i32;
                yy = yy.max(*left.value(0, m));
                yy = yy.max(*right.value(m, i + 1) + (i + x) as i32);
                y = y.min(yy);
            }
...
// 149568948	Mar/13/2022 17:49UTC-7	watashi	1632E1 - Distance Tree (easy version)	Rust 2021	Accepted	46 ms	1300 KB
